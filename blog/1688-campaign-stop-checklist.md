...
规则先行：学习期保护**不是固定周数**——只有首个定版周、且连续投放、样本不足（询盘加线索不足 5 条）才触发，最多再等 1 周；从第二个定版周起，永不保护。之后每周都按同一把尺子量：有效获客成本超目标、相对差距超过 25%，且询盘占比低（低于 15%，或不到全店水平的七成）、或消耗走势无改善（近 2 周环比涨超 12% 且成本超标，或 3 周斜率向上）→ 进停止档；差距超过 50% 且成本超标的，判得更快。
